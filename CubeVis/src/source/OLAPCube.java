 ...
 */
public class OLAPCube extends JPanel implements MouseListener, MouseMotionListener{
    ...
    String segundo_dataset="Segundo";
     ...
     public OLAPCube(DBSQLConnect dbConn, DBSQLStatement dbStat, Vector dimSize, Vector DVector, Vector MVector, Vector values, String tblname, String idtable, int test, OlapGraphicsData graphic, OlapGraphicsData graphic2, int numCube, String posCube, OLAPStatGraphics o,OlapGraphicsData graphic3 ) {
	...
	objScale.addChild(objTrans1);
        ...
        {
        //Creando diferencias significativas para probar
            System.out.println("Procedimiento mas de 3 dimensiones");
            createSigDiffs ();
            System.out.println("Fin de Procedimiento mas de 3 dimensiones");
        }
  ...
  { 
		...
		while (results.length > 0)
		{
			...
			results = DBC.getResult (names);
                ...
                {
                System.out.println("Valores de start:"+(String)start.elementAt(x));
                
                }
                
                for(int x=0; x < finish.size(); x++)
                {
                System.out.println("Valores de finish:"+(String)finish.elementAt(x));
                
                }
                
                
		System.out.println ("END OF RESULTS OLAPCUBE");
  }

    
    //Agregando metodos para recuperar vector con relaciones 
    public Vector vectorStart(){
      return this.start;
    } 
     
    public Vector vectorFinish(){
       return this.finish;
    }
        ...
        if (result == null) {
            ...
            try{
                ...
                if(start.contains(coor)){
                   ...
                   System.out.println("Encontre una relacion y es:"+relation);
